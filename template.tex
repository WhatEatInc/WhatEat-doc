...
\newenvironment{TreeStruct}[1]{\begin{tcolorbox}[enhanced jigsaw, arc=0pt, boxrule=0.2pt, opacityback=0pt, title={#1}]}{\end{tcolorbox}}
...
\newpairofpagestyles{custom-header-footer}{
  ...
  \ohead*{$if(header-right)$ $header-right$ $else$ \today $endif$ }
...
$if(custom-title-page)$
    \pagestyle{empty}
	\begin{center}
        $if(custom-title-page-logo)$
        \includegraphics[height=90mm]{$custom-title-page-logo$}
        $endif$
